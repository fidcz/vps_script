#!/bin/sh
#=========================================================
# Alpine Linux 专用: Trojan-Go 极简一键安装脚本
# 适配 128M 内存 LXC 架构，支持自定义/随机密码
#=========================================================
...
# 2. 收集用户输入 (密码)
read -p "请输入 Trojan-Go 密码 (直接回车将随机生成): " PASSWORD
if [ -z "$PASSWORD" ]; then
    # 如果用户没有输入密码，则利用 openssl 生成 16 位随机字符
    PASSWORD=$(openssl rand -hex 8)
    echo " -> 检测到密码为空，已为您生成随机密码: $PASSWORD"
fi

echo ""
echo "[1/6] 正在安装必要依赖 (curl, unzip, openssl)..."
apk update >/dev/null 2>&1
apk add --no-cache curl unzip openssl >/dev/null 2>&1

echo "[2/6] 正在为 bing.com 生成 10年有效期自签证书..."
...
echo "[3/6] 正在下载并配置 Trojan-Go 二进制文件..."
...
echo "[4/6] 正在生成 config.json 配置文件..."
...
echo "[5/6] 正在注册 OpenRC 开机自启服务..."
# 注意：这里使用 'EOF' 带单引号，防止 $RC_SVCNAME 变量被 shell 提前解析
...
echo "========================================================="
echo " 请在您的客户端（如 v2rayN / Shadowrocket）中按以下信息配置："
echo ""
...
echo "========================================================="
echo " 管理命令提示:"
echo " 启动服务: rc-service trojan-go start"
echo " 停止服务: rc-service trojan-go stop"
echo " 查看状态: rc-service trojan-go status"
echo " 查看报错: cat /var/log/trojan-go.err"
echo "=========================================================
